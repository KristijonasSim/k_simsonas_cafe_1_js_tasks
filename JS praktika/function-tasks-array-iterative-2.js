...
{ 
   const studentsAverages = students.map(({modules, ...rest}) => ({
     ...rest,
     modules,
   ...
   console.table(studentsAverages);
...
console.groupCollapsed('5. Atspausdinti visų Informatikos fakulteto studentų vidurkius');
{
  ...
  const ItStudentsAverages = ItStudents.map(({modules, ...rest}) => ({
    ...rest,
    modules,
    avg:modules.reduce((result, {credits, marks}) =>
    result + credits * marks.reduce((acc, mark) => acc + mark) / marks.length
  ...
  console.table(ItStudentsAverages);
...
{
  const ChemistryStudents = students.filter(({faculty}) => faculty === `Chemijos fakultetas`)
  const ChemistryStudentsAverages = ChemistryStudents.map(({modules, ...rest}) => ({
    ...rest,
    modules,
    avg:modules.reduce((result, {credits, marks}) =>
    result + credits * marks.reduce((acc, mark) => acc + mark) / marks.length
  ...
  console.table(ChemistryStudentsAverages);
...
{
  const ElectricalAndElectronicsStudents = students.filter(({faculty}) => faculty === `Elektros ir elektronikos fakultetas`)
  const ElectricalAndElectronicsStudentsAverages = ElectricalAndElectronicsStudents.map(({modules, ...rest}) => ({
    ...rest,
    modules,
    avg:modules.reduce((result, {credits, marks}) =>
    result + credits * marks.reduce((acc, mark) => acc + mark) / marks.length
  ...
  console.table(ElectricalAndElectronicsStudentsAverages);
